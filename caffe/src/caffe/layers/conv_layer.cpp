...
void RotInvConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK_EQ(num_spatial_axes_, 2)
        << "RotInv only supports 2D Convolutions. I can't wrap my head around"
  ...
  //Also ensure that the kernel is square
  for (int i = 0; i < num_spatial_axes_; ++i) {
...
template <typename Dtype>
void RotInvConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const int first_spatial_axis = channel_axis_ + 1;
  ...
  CHECK_EQ(bottom[1]->shape(1), 1)
      << "orientation map would have 1 channel, just radians.";
  // Shape the tops.
...
template <typename Dtype>
void RotInvConvolutionLayer<Dtype>::compute_output_shape() {
...
template <typename Dtype>
void RotInvConvolutionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void RotInvConvolutionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(RotInvConvolutionLayer);
#endif

INSTANTIATE_CLASS(RotInvConvolutionLayer);
